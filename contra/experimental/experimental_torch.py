...
class ExperimentalModule(pl.LightningModule):
    ...
    def step(self, batch, name):
        ...
        #     print(f'ytrue: {ytrue} shape: {ytrue.shape}, ypred: {ypred} shape: {ypred.shape}')
        #     # When the batch is of size 1, squeeze results in a scalar.
        #     loss = self.loss_func(ypred.unsqueeze(0), ytrue)
        # else:
...
class SimpleClassifier(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim):
        ...
        layers = [nn.Linear(in_dim, hid_dim), act_func, nn.Linear(hid_dim, out_dim), nn.Sigmoid()]
...
class W2VClassifier(ExperimentalModule):
    def __init__(self, emb_size, hparams):
        ...
        self.w2v = PretrainedW2V(idf_path=os.path.join(FULL_PUMBED_2020_PATH, 'idf_dict2010_13+16_18_train.pickle'),
                                 vectors_path=os.path.join(SAVE_PATH, 'word2vec_2010_13+16_18_train.wordvectors'),
    ...
    def forward(self, batch):
        embedded = self.w2v.embed_batch([self.tu.word_tokenize(sample) for sample in batch['text']], device=self.device)
...
class W2V3SentClassifier(W2VClassifier):
    ...
    def three_sents_to_wordlist(self, three_sent_as_text):
        wordlist = []
        for sent in three_sent_as_text.split('<BREAK>'):
            wordlist.extend(self.tu.word_tokenize(sent))
        return wordlist

    def forward(self, batch):
        embedded = self.w2v.embed_batch([self.three_sents_to_wordlist(sample) for sample in batch['text']], device=self.device)
        y_pred = self.linear(embedded)
        return y_pred
...
class Bert3SentClassifier(ExperimentalModule):
    def __init__(self, hparams, mode='mean'):
        ...
        # self.classifier = SimpleClassifier(classifier_size, 10, 1)
    ...
    def forward(self, x):
        ...
        if self.mode == 'ensemble':
            sent_pred = torch.sigmoid(self.classifier(sent_embedding))
...
class Bert3SentTransformerClassifier(Bert3SentClassifier):
    ...
    def forward(self, x):
        ...
        # after the first transpose the shape will be: tokens*batch_size*emb_dim.
...
class Bert3SentWordMeanClassifier(Bert3SentClassifier):
    ...
    def forward(self, x):
        ...
        inputs = {k: v.to(self.device) for k, v in inputs.items()}
        outputs = self.bert_model(**inputs, output_hidden_states=False)
        # TODO: test tf idf weighting
        idf_per_token = inputs['input_ids'].clone().apply_(lambda tid: self.token_id_to_idf[tid])
...
hparams = config.parser.parse_args(['--name', '3sent_noOL_w2v-tfidf_logreg',
                                    #'--name', '1sent_w2v-tfidf_logreg',
...
if __name__ == '__main__':
    ...
    model = W2V3SentClassifier(emb_size=300, hparams=hparams)
