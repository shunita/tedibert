...
if __name__ == '__main__':
    ...
    # generate more admissions based only on female patients from the train, without patients who died in the hospital.
    ...
    for i, row in admissions.iterrows():
        for field in field_to_mapping:
            for item in row[field]:
                ...
                if field == 'ICD9_code': # take only the first diagnosis (primary)
                    break
    ...
    print(f"ICD9_CODES: {len(field_to_mapping['ICD9_CODE'])}, PREV_DIAGS:{len(field_to_mapping['PREV_DIAGS'])}")
