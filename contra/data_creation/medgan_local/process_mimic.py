...
if __name__ == '__main__':
    ...
    types = {} # mapping from code to matrix column index
    newSeqs = []
    for patient in seqs:
        newPatient = []
    ...
    # This makes a patients * codes matrix. For our tasks we want an admissions * codes matrix.
    # Issue: when a new "admission" is generated, would we generate the patient features as well?
        ...
        for visit in patient:
            for code in visit:
                if binary_count == 'binary':
                    ...
                    matrix[i][code] += 1.
    # only the matrix is used in the medgan training/generation. The pids and types are for processing - I'm guessing, to be able to trace back the conditions and patients.
