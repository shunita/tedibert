...
class PubMedExpModule(PubMedModule):
    ...
    def setup(self, stage=None):
        ...
        elif self.serve_type == 6:
            df = pd.read_csv(os.path.join(DATA_PATH, 'pubmed2020_with_bert_embedding.csv'), index_col=0)
            if self.hparams.debug:
                df = df.sample(1000)
            df = fill_binary_year_label(df, self.first_time_range, self.second_time_range)
            df['bert_emb_np'] = df['bert_emb_np'].apply(make_array)
            self.train_df = df[df['assignment'] == 0].copy()
            self.val_df = df[df['assignment'] == 1].copy()
            print(f"special debug mode: 6, loaded {len(self.train_df)} train samples and {len(self.val_df)} test samples")
            ytrain = self.train_df['label'].values
            yval = self.val_df['label'].values
            self.train = PubMedExp3SentBertDataset(self.train_df, ytrain)
            self.val = PubMedExp3SentBertDataset(self.val_df, yval)
...
class PubMedExp3SentBertDataset(PubMedExp3SentDatasetBase):
    def __init__(self, df, y, context_field_name='sentences', sentence_field_name='text'):
        super(PubMedExp3SentBertDataset, self).__init__(df, y, context_field_name, sentence_field_name)

    def __getitem__(self, index):
        line = self.df.iloc[index]
        return {'text': line['bert_emb_np'],
                'is_new': self.y[index]}
